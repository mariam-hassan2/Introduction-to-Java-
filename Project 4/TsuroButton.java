	...
	public class TsuroButton extends Button {
	  /** An array that stores the endpoint to endpoint connections of the 4 paths on 
	the icon */
	  ...
	  /**
	   * Change the connections for the paths of the game tile.  The paths are drawn on
	the button icon.  A null input produces a blank tile.
	   * @param connections  an array of 8 ints.  connections[i] = j means there is a 
	path from endpoint i to endpoint j.
	   */
	  ...
	  /**
	   * Return an array showing the connections for the game tile that are drawn on 
	the icon
	   * @return  the array giving the endpoint connections for the paths that are 
	drawn on the icon
	   */
	  ...
	  /** Add a pair to a random symmetric permutation.
	    ...
	    */
	  private static void makeRandomConnection(int[] connections, boolean[] filled, int
	numLeft) {
	    int next = 0;
	    while (filled[next])                                 // get the next spot not 
	yet connected
	      next++;
	    
	    int skip = (int)(Math.random() * (numLeft - 1));     // randomly choose from 
	the remaining spots
	    int connectTo = next + 1;
	    while (skip > 0 || filled[connectTo]) {              // go to that randomly 
	chosen spot
	      if (!filled[connectTo])
	        skip--;
	      connectTo++;
	  ...
	  /**
	   * Create a random symmetric permutation.  The permutation represents the 
	connections between
	   * two endpoints.  If index i stores value j, then the icon will have a path from
	endpoint i to
	   * endpoint j.
	...
	*/
	  private class GameTile extends Canvas {
	    ...
	    /**
	     * Returns the end point location (in pixels) of a particular end point of a 
	path on the tile
	     * @param value a value between 0-7 to indicate one of the 8 path endpoints on 
	this tile
	     */
	    ...
	    private void drawPath(GraphicsContext gc, int start, int end) {
	      ...
	      if (start_dim.getWidth() == 0 && end_dim.getWidth() == 0)
	        gc.strokeArc(-getWidth()/4, getHeight() / 3, getWidth() / 2, 2*getHeight()/
	3 - getHeight() / 3, 270, 180, javafx.scene.shape.ArcType.OPEN);
	      else if (start_dim.getWidth() == getWidth() - 1 && end_dim.getWidth() == 
	getWidth() - 1)
	        gc.strokeArc(getWidth() - 1 - getWidth() / 4, getHeight() / 3, getWidth() /
	2, 2*getHeight()/3 - getHeight() / 3, 90, 180, javafx.scene.shape.ArcType.OPEN);
	      else if (start_dim.getHeight() == 0 && end_dim.getHeight() == 0)
	        gc.strokeArc(getWidth() / 3, - getHeight() / 4, 2*getWidth() / 3 - 
	getWidth() / 3, getHeight() / 2, 180, 180, javafx.scene.shape.ArcType.OPEN);
	      else if (start_dim.getHeight() == getHeight() - 1 && end_dim.getHeight() == 
	getHeight() - 1)
	        gc.strokeArc(getWidth() / 3, getHeight() - 1 - getHeight() / 4, 2 * 
	getWidth() / 3 - getWidth() / 3, getHeight() / 2, 0, 180, 
	javafx.scene.shape.ArcType.OPEN);
	      else
	        gc.strokeLine(start_dim.getWidth(), start_dim.getHeight(), 
	end_dim.getWidth(), end_dim.getHeight());
	    }
